...
1. **Pre-Reservation Stock Checks**: Before mutating state, `OrderService` checks all requested items against current stock via `inventoryService.getItem()`. If any item exceeds available stock, the transaction aborts before altering any inventory, eliminating partial reservations.
2. **Local ACID Transactions**: The `@Transactional` boundary on `placeOrder()` ensures all inventory decrements and order record insertions share a single database transaction. If an unexpected runtime exception occurs, Spring and Hibernate trigger a database rollback, keeping data strictly consistent.

If Order and Inventory were split across a network, shared database transactions would be impossible. We would need to implement an **Orchestrated or Choreographed Saga pattern**. The Order service would issue reservation requests across HTTP/gRPC. If an item fails midway, a saga coordinator must execute **compensating transactions** (calling a restock endpoint) to release previously reserved inventory. This requires idempotency keys, persistent saga logs, and eventual consistency handling to recover from transient network drops.

### 2. Event-Driven Decoupling and Microservice Integration for Notifications
Publishing domain events (`OrderPlacedEvent`, `OrderRejectedEvent`, `OrderCancelledEvent`) via Spring’s `ApplicationEventPublisher` eliminates direct coupling between `OrderService` and `Notification`. Instead of synchronous method invocations, `OrderService` simply publishes facts about domain state. It possesses zero knowledge of who subscribes, how many listeners exist, or whether notifications succeed.

If Notification became a separate microservice:
- **Message Broker**: An external broker like RabbitMQ or Apache Kafka would replace Spring's in-process event bus to transport events asynchronously across the network.
- **Delivery Guarantees**: To achieve reliable at-least-once delivery, the Order service would implement the **Transactional Outbox Pattern** (persisting events to an outbox table within the order transaction, published via a CDC tool like Debezium) to prevent message loss during crashes.
- **Consumer Idempotency**: The Notification microservice would require idempotent consumers tracking processed event IDs to avoid duplicate alerts during network redeliveries, backed by Dead Letter Queues (DLQ) for malformed payloads.

### 3. First Microservice Extraction: The Notification Module
If forced to extract exactly one module first, **Notification** is the optimal choice:
- **Non-Blocking Flow**: Notification is purely a downstream consumer. Neither Order nor Inventory awaits a response from it, eliminating latency risks in the critical checkout path.
- **Fault Isolation**: If the Notification service goes down, core customer operations (browsing inventory, placing orders, and cancelling orders) remain completely operational.
- **Pre-Existing Event Boundary**: Notification already communicates solely through immutable domain events without direct service calls.

**Required Code Changes**:
- In **Shop** and **Inventory**, replace in-memory `eventPublisher.publishEvent()` calls with a message producer (e.g., Spring `KafkaTemplate`) to publish domain event JSON to message topics.
- In **Notification**, create an independent Spring Boot service with its own database and replace `@EventListener` in `NotificationEventListener` with `@KafkaListener(topics = "order-events")`.
- Migrate the `notifications` table to an isolated database schema.
- **Database**: Move the `notifications` table to a separate PostgreSQL database instance, completing physical and logical service decoupling.
